...
class TestIterableProtocol(unittest.TestCase):
    ...
    def test_iter(self):
        ...
        # use a lambda so the assertion calls the next(i) rather than
        # our code calling next(i) and throwing the exception
        self.assertRaises(StopIteration, lambda: next(i))

    # exercise the index iterator
...
class TestSequenceProtocol(unittest.TestCase):
    ...
    # we're using a set, so there's only ever going to be
    ...
    # basically testing the "addition"
    ...
    # these are for the "*" repetition operator
    # remember, this is a set, so really there's either 0 or 1
    # of something
...
class TestReprProtocol(unittest.TestCase):
    def test_repr_empty(self):
        s = SortedSet()
        # equality, from the base object, is reference equality
...
class TestEqualityProtocol(unittest.TestCase):
    ...
    def test_identical_via_not(self):
        s = SortedSet([10,11,12])
        s2 = SortedSet([11,12,13])
